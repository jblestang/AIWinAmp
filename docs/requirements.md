...
| R4 | The player shall expose a master volume slider (0-100%) and pre-set equalizer bands that can be adjusted, clamped, and reset. | Should | T4 (`src/equalizer.rs::tests::clamps_gain_values`, `resets_to_flat`) | Equalizer is simulated but stateful for UI parity. |
